...
T8_NODE_NAME="minimax-h3-audio-T8"
T8_LEGACY_NODE_NAME="comfyui-minimax-h3-audio-T8"
...
h3_studio_install_runtime_nodes() {
  ...
  local t8_legacy="$COMFY_DIR/custom_nodes/$T8_LEGACY_NODE_NAME"
  mkdir -p "$COMFY_DIR/custom_nodes"

  # T8's Director dynamically imports
  # /extensions/minimax-h3-audio-T8/director/workbench.mjs. ComfyUI derives
  # WEB_DIRECTORY route names from the custom_nodes folder basename, so keep
  # the upstream-recommended folder name exactly. Migrate the folder used by
  # earlier Open Studio builds before ComfyUI is restarted.
  if [[ -e "$t8_legacy" && "$t8_legacy" != "$t8_target" ]]; then
    if [[ -e "$t8_target" ]]; then
      if [[ -d "$t8_legacy/.git" ]] && git -C "$t8_legacy" remote get-url origin 2>/dev/null | grep -Eq '(^|/)(T8mars/)?comfyui-minimax-h3-audio-T8(\.git)?
...
h3_studio_verify_t8_director() {
  "$COMFY_PYTHON" - \
    ...
    "http://127.0.0.1:$COMFY_PORT/extensions/minimax-h3-audio-T8/director/workbench.mjs" <<'PY'
import json
import sys
import urllib.request

object_info_url, ui_url, capabilities_url, workbench_url = sys.argv[1:]
...
try:
    with urllib.request.urlopen(workbench_url, timeout=20) as response:
        workbench = response.read().decode("utf-8", "replace")
except Exception as exc:
    print(
        "[ERROR] Could not load T8 Director extension module. "
        "The custom_nodes folder must be named minimax-h3-audio-T8: "
        + str(exc),
        file=sys.stderr,
    )
    raise SystemExit(1)

if "createDirectorWorkbench" not in workbench:
    print("[ERROR] T8 Director workbench module returned unexpected content.", file=sys.stderr)
    raise SystemExit(1)
...
print("[OK] T8 Obsidian Director node, UI, extension module and capabilities routes are ready", file=sys.stderr)
PY
}
...
; then
        rm -rf "$t8_legacy"
        h3_profile_info "Removed legacy T8 checkout path after canonical target already existed."
      else
        h3_profile_error "Legacy T8 path conflicts with canonical install and is not a managed checkout: $t8_legacy"
        return 1
      fi
    elif [[ -d "$t8_legacy/.git" ]] && git -C "$t8_legacy" remote get-url origin 2>/dev/null | grep -Eq '(^|/)(T8mars/)?comfyui-minimax-h3-audio-T8(\.git)?
...
main_studio "$@"
; then
      mv "$t8_legacy" "$t8_target"
      h3_profile_info "Migrated T8 checkout to canonical extension folder: $T8_NODE_NAME"
    else
      h3_profile_error "Legacy T8 path is not a managed checkout and cannot be migrated safely: $t8_legacy"
      return 1
    fi
  fi
